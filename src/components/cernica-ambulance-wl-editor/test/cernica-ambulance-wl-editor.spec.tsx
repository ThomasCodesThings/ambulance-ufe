...
describe('cernica-ambulance-wl-editor', () => {
 ...
 afterEach(() => { mock.reset(); });
  ...
  it('buttons shall be of different type', async () => {
    mock.onGet(/^.*\/entries\/.+/).reply(200, sampleEntry);
     ...
     const page = await newSpecPage({
      ...
      html: `<cernica-ambulance-wl-editor entry-id="test-entry"
  ...
  it('first text field is patient name', async () => {
    ...
    const page = await newSpecPage({
       components: [CernicaAmbulanceWlEditor],
       html: `<cernica-ambulance-wl-editor entry-id="test-entry" ambulance-id="test-ambulance" api-base="http://sample.test/api"></cernica-ambulance-wl-editor>`,
    });
